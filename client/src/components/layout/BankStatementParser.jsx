...
/**
 * BankStatementParser - Smart transaction import from multiple sources
 * 
 * PS.md Alignment:
 * - Supports bank statements, SMS, and email sources
 * - AI-powered parsing and categorization
 * - Preview before import for user control
 */
const BankStatementParser = ({ onClose, onParseSuccess }) => {
  ...
  const handleConfirmImport = async () => {
    setParsing(true);
    try {
      const fileName = file?.name || `${sourceType}_paste_${Date.now()}`;
      ...
      if (onParseSuccess) {
        // Pass file info for tracking in sidebar
        const fileInfo = { name: fileName, type: sourceType };
        onParseSuccess(response, fileInfo);
      }
    } catch (err) {
          ...
          {result?.imported ? (
            ...
            <>
              ...
              <div className="flex gap-3 mt-4">
                ...
                <button
                  onClick={handleParse}
                  disabled={parsing || (!file && !textContent.trim())}
