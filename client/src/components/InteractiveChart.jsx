...
export const InteractiveBarChart = ({ data, title, onDrillDown }) => {
  ...
  const handleBarClick = (entry) => {
    ...
    if (onDrillDown) {
      ...
      onDrillDown(`Show me all ${entry.name}'s transactions`);
    }
  };
      ...
      <div className="grid grid-cols-2 gap-2 mt-4">
        {data.data.map((item, index) => (
          ...
          >
            ...
            />
            <span className="text-dark-400 flex-1 text-left">{item.category}</span>
...
// Interactive Timeline with drill-down on specific dates
