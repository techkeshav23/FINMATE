...
// Intent definitions with semantic variations
const INTENT_PATTERNS = {
  spending_analysis: {
    keywords: ['spent', 'spending', 'expenses', 'paid', 'cost', 'money spent', 'how much'],
    semantic: ['who paid more', 'expense breakdown', 'money going out', 'cash flow', 'expenditure'],
    ambiguous: ['spending', 'expenses', 'money'] // Needs clarification
  },
  settlement: {
    keywords: ['owe', 'owes', 'debt', 'settle', 'settlement', 'pay back', 'balance'],
    semantic: ['who should pay whom', 'clear up', 'even out', 'square up', 'split the bill'],
    ambiguous: []
  },
  category_breakdown: {
    keywords: ['category', 'breakdown', 'distribution', 'split by', 'by type'],
    semantic: ['where is money going', 'spending categories', 'expense types', 'categorize'],
    ambiguous: ['breakdown']
  },
  unpaid_bills: {
    keywords: ['unpaid', 'pending', 'unsettled', 'outstanding', 'due'],
    semantic: ['bills to pay', 'not yet paid', 'remaining bills', 'open transactions'],
    ambiguous: []
  },
  timeline: {
    keywords: ['timeline', 'trend', 'over time', 'history', 'graph', 'chart'],
  ...
  comparison: {
    ...
    semantic: ['this month vs last', 'week over week', 'period comparison', 'changed since'],
    ambiguous: []
  },
  anomaly: {
    keywords: ['unusual', 'anomaly', 'weird', 'strange', 'spike', 'abnormal'],
    semantic: ['something wrong', 'looks off', 'unexpected', 'out of ordinary', 'red flag'],
    ambiguous: []
  },
  simulation: {
    keywords: ['what if', 'simulate', 'plan', 'budget', 'project', 'forecast'],
    semantic: ['hypothetical', 'scenario', 'if we', 'planning ahead', 'predict'],
  ...
  filter: {
    keywords: ['show', 'list', 'filter', 'only', 'just'],
    semantic: ['zoom in', 'focus on', 'specific transactions', 'narrow down'],
    ambiguous: []
  }
};
...
const CLARIFYING_QUESTIONS = {
  spending: {
    ...
    options: [
      { label: "By person", intent: "spending_by_person", query: "Who spent the most?" },
      { label: "By category", intent: "spending_by_category", query: "Show category breakdown" },
      { label: "Over time", intent: "spending_timeline", query: "Show spending timeline" },
      { label: "All transactions", intent: "all_transactions", query: "Show all transactions" }
    ]
  },
  expenses: {
    question: "What aspect of expenses would you like to explore?",
    options: [
      { label: "Total by roommate", intent: "spending_by_person", query: "Who paid how much?" },
      { label: "By category", intent: "spending_by_category", query: "Break down by category" },
      { label: "Pending only", intent: "unpaid_bills", query: "Show unpaid bills" },
      { label: "Recent trends", intent: "spending_timeline", query: "Show spending trend" }
    ]
  },
  money: {
    question: "I can help with several money-related questions. Which one?",
    options: [
      { label: "Who owes whom?", intent: "settlement", query: "Who owes money to whom?" },
      { label: "Spending breakdown", intent: "spending_by_person", query: "Show spending by person" },
      { label: "Pending bills", intent: "unpaid_bills", query: "Show unpaid bills" },
      { label: "Budget planning", intent: "simulation", query: "Plan next month's budget" }
    ]
  },
  breakdown: {
    ...
    options: [
      ...
      { label: "By person", intent: "spending_by_person", query: "Show spending by person" },
  ...
  trend: {
    ...
    options: [
      ...
      { label: "Category changes", intent: "comparison", query: "Compare this month vs last" },
  ...
  plan: {
    ...
    options: [
      { label: "Next month's budget", intent: "simulation_budget", query: "Plan next month's budget" },
      { label: "Settle up now", intent: "settlement", query: "How should we settle up?" },
      { label: "What-if scenario", intent: "simulation", query: "What if rent increases by 10%?" }
    ]
  }
};
...
const analyzeWithGemini = async (query, context) => {
  ...
  try {
    ...
    const prompt = `You are an intent classifier for a roommate expense tracking app.

Classify this user query into ONE of these intents:
- spending_by_person: Who spent how much
- spending_by_category: Category breakdown
- settlement: Who owes whom, debts
...
Context: ${context ? JSON.stringify(context) : 'Roommate expense tracking app'}
